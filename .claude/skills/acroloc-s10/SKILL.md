...
- **Recovery is `T200 M6`, not a change back to the same tool.** CNC12 skips an M6 for the tool its status window names, and that is read-only to us, so after a hand move a change back to that tool is silently ignored. Tool 200 is a dummy mapped to a bin in the Tool Library; the VCP **ATC RESET** button and wireless MPG macro button 4 (`system/plcmacro4.mac`) both run `T200 M6`, which cannot be skipped. The PLC posts message 175 when a reset is owed and 176 when the position is proven.
