...
`Centroid-Acroloc-ALLIN1DC.src:1196`; `GearShiftStage IS STG17 ; Acroloc RPM gear-shift state
machine`, `Centroid-Acroloc-ALLIN1DC.src:1208`) and then swept top to bottom, once per scan,
...
arms `GearCoast_T` and then `SET GearShiftStage` (`Centroid-Acroloc-ALLIN1DC.src:2307`,
inside the block commented "so a shift always completes ... the coast timer GearCoast_T is
loaded and armed by the kickoff rung in MainStage, in the same scan this stage is SET",
`Centroid-Acroloc-ALLIN1DC.src:2995`). `GearShiftStage` itself is `STG17`
(`Centroid-Acroloc-ALLIN1DC.src:1208`), which is swept after `MainStage`'s `STG4` in the same
pass, so `GearShiftStage`'s Step A rung (`Centroid-Acroloc-ALLIN1DC.src:2999`) runs in that
...
(`Centroid-Acroloc-ALLIN1DC.src:3004`-3005, timer read at
`Centroid-Acroloc-ALLIN1DC.src:3007`). In other words, `SET GearCoast_T` in
`MainStage` (`Centroid-Acroloc-ALLIN1DC.src:2306`) does not make `GearCoast_T` read true in
...
Stock example — `LubeM_T IS T13` (`Centroid-Acroloc-ALLIN1DC.src:1180`):
- `Centroid-Acroloc-ALLIN1DC.src:1422`: `IF !(SV_PROGRAM_RUNNING || SV_MDI_MODE) THEN
  LubeM_T = LubeM_W, SET LubeM_T` — loads the preset from `LubeM_W` and arms it.
- `Centroid-Acroloc-ALLIN1DC.src:1423`: `IF LubeM_T || !EStopOk_M THEN RST Lube_O` — reads
  the bare timer (true at expiry) to gate an output.
- `Centroid-Acroloc-ALLIN1DC.src:1421`: `RST LubeM_T` — zeroes it when the program is running.

Acroloc example — `GearCoast_T IS T25 ; Acroloc gear-shift coast dwell (neutral) before
engage` (`Centroid-Acroloc-ALLIN1DC.src:1189`):
- `Centroid-Acroloc-ALLIN1DC.src:2301`/2304: `GearCoast_T = 1500` (default) or `GearCoast_T =
  SV_MACHINE_PARAMETER_943` (configured) loads the preset.
- `Centroid-Acroloc-ALLIN1DC.src:2306`: `SET GearCoast_T` arms it, in `MainStage`, the same
  scan `GearShiftStage` is set.
- `Centroid-Acroloc-ALLIN1DC.src:3007`: `IF GearShiftStage && GearCoast_T && (DesiredRange_W
  == 1) THEN ...` reads the bare (expired) timer to gate clutch engagement.
- `Centroid-Acroloc-ALLIN1DC.src:3015`: `RST GearCoast_T` zeroes it once the shift finishes.
...
- `Centroid-Acroloc-ALLIN1DC.src:2999`: Step A unconditionally does `RST
  Spindle_Low_gear_O, RST Spindle_High_gear_O` every scan the stage is set (open the clutches
  to neutral).
- `Centroid-Acroloc-ALLIN1DC.src:3008`: Step B conditionally does `SET Spindle_Low_gear_O,
...
- `_I` = input bit (`INP`) — `ZeroSpeed_I IS INP12` (`Centroid-Acroloc-ALLIN1DC.src:219`).
- `_O` = output bit (`OUT`) — `ATCMotor_O IS OUT17 ; Acroloc spin tool carousel`
  (`Centroid-Acroloc-ALLIN1DC.src:382`).
- `_M` = memory bit (`MEM`) — `SafetySwitch_M IS MEM29`
  (`Centroid-Acroloc-ALLIN1DC.src:486`).
- `_W` = 32-bit word (`W`) — `ChangeToTool_W IS W72 ; Acroloc`
  (`Centroid-Acroloc-ALLIN1DC.src:1094`).
- `_FW` = floating-point word — `RPMPerBit_FW IS FW2`
  (`Centroid-Acroloc-ALLIN1DC.src:1102`).
...
- `_SV` = CNC system variable — `DoCycleCancel_SV IS SV_PLC_FUNCTION_1`
  (`Centroid-Acroloc-ALLIN1DC.src:770`).
- `_C` = integer constant, message constants encode as `value = msgNumber + 256*msgFile` —
  ...
  (`Centroid-Acroloc-ALLIN1DC.src:202`).

Stages use no suffix and are declared `IS STGn` — e.g. `MainStage IS STG4`
(`Centroid-Acroloc-ALLIN1DC.src:1196`).
