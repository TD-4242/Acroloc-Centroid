...
| P700 | Requested tool number of the last M6, written by `mfunc6.mac` (`G10 P700 R[#4120]`) | `ToolInSpindleDisp_W = SV_MACHINE_PARAMETER_700` in the `ATCStage` match rung (unpinned) | [atc.md](atc.md) | Set by the macro, never by hand; feeds the VCP `TOOL` readout. Labelled in `language.msg` |
